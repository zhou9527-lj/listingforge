...
0. **当前状态速览（2026-08-10 第二十五次）**：任务中心排版修复完成（表格从固定 54px 行释放回 `1fr` 行、窄窗口自动隐藏「项目/用时」列、分页栏固定底部），`npm run check` 全绿 + click-smoke PASS，已提交推送；新增布局诊断脚本 `scripts/task-layout-shot.mjs` 供排版回归检查。正式 Release `v0.1.4` 已发布（见第二十四次）。待人工推进：真 Key 三供应商联网验收（余额/费用列才出真实数字）、SignPath Foundation 申请、macOS 公证。
