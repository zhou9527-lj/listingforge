...
0. **当前状态速览（2026-08-10 第二十六次）**：任务中心排版修复（第二十五次）已升版 `0.1.5` 并触发三端构建（标签 `build-0.1.5-tasks-layout`），构建成功后发布正式 Release `v0.1.5`（记录见本文件末尾与发布方案）。正式 Release `v0.1.4` 已发布（见第二十四次）。待人工推进：真 Key 三供应商联网验收（余额/费用列才出真实数字）、SignPath Foundation 申请、macOS 公证。
